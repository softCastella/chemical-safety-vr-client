...
## 2026-09-22 Meta Horizon 정식 출시 이후 피드백 및 업데이트 계획

### 회의 목적과 기준

- Meta Horizon 정식 출시 이후 확인된 사용자 피드백, 영문판 준비 범위와 다음 업데이트의 역할을
  한 기준으로 정리한다.
- 클라이언트 기준은 `main@6dbfb2eadc56`, 서버 기준은 `main@0839002234fe`이다. 이번 회의록 작성은
  클라이언트·서버 런타임 코드나 API 계약을 변경하지 않는다.
- 이 절의 Meta 출시 시각과 채널 상태는 사용자가 Meta Horizon 화면에서 확인해 전달한 내용이다.
  이번 문서 작업에서 Meta Developer Dashboard에 다시 로그인해 독립 검증하지 않았으므로
  `사용자 확인`과 `저장소 확인`을 구분한다.

### 출시 및 빌드 상태

| 항목 | 현재 상태 | 확인 수준 |
| --- | --- | --- |
| 정식 출시본 | `1.0.0 / versionCode 8` | 사용자가 2026-09-17 10:18 KST에 출시 처리됐다고 확인 |
| DUC 보완 빌드 | `1.0.0 / versionCode 9` | Alpha 업로드까지만 완료된 상태로 관리하며 Production 출시로 합쳐 쓰지 않음 |
| 로컬 Unity 설정 | `bundleVersion: 1.0.0`, `AndroidBundleVersionCode: 9` | `ProjectSettings/ProjectSettings.asset` 정적 확인 |
| 다음 업로드 코드 | `versionCode 10` | 이미 code 9가 업로드됐으므로 다음 바이너리는 반드시 더 큰 값 사용 |
| 다음 표시 버전 | 아래 두 출시안 중 결정 필요 | 아직 Project Settings에 적용하지 않음 |

- 기존 2026-09-12 절의 `Release Build 10`은 당시 제출 잠금 상태에서 작성한 계획이다. 이후 사용자가
  확인한 2026-09-17 정식 출시 사실을 최신 운영 기준으로 사용하되, code 8과 code 9의 채널 상태는
  계속 분리한다.
- 모바일 Meta Horizon 상세페이지에 개인 이름이 표시된 원인은 앱의 `Publisher Name`이 아니라
  팀 이름이었다고 사용자가 확인했고, 팀 이름을 회사명으로 수정했다. 모바일 상세페이지의 캐시 갱신과
  최종 회사명 표시는 별도로 다시 확인한다.
- `C:\Users\lanoc\Downloads\Tyche_works_circle_Logo_512.png`에 512×512 PNG 사본을 만들었다.
  저장소 밖의 로컬 작업 파일이므로 Meta 페이지 업로드·승인 완료 증거로 사용하지 않는다.

### 정식 출시 후 피드백 범위

| 번호 | 피드백 또는 요구 | 현재 결정 | 구현 방향 |
| --- | --- | --- | --- |
| 1 | 그림자가 없어 실제 공간과 사용자 존재감이 약함 | 반영 후보 | 룸·가구의 정적 그림자와 보이지 않는 사용자 그림자를 분리해 구현 |
| 2 | PPE를 입는 순간 컨트롤러 진동 필요 | 반영 후보 | 실제 착용 완료가 확정된 순간 선택에 사용한 컨트롤러에 짧은 햅틱 1회 |
| 3 | 체크리스트를 작게 만들어 좌측에서 따라오게 하고 미니 컨트롤러를 우측으로 이동 | 반영 후보 | 씬·Inspector 작성값을 기준으로 크기·좌우 위치·추적 강도 조정 |
| 4 | 구체적인 전신 착용 애니메이션 | 이번 범위 제외 | 기술·일정 제약으로 구현하지 않고 햅틱과 착용 결과 표현으로 보완 |
| 5 | 영어 버전 준비 | 기획 및 구현 대상 | UI 텍스트·로고·이미지·TTS를 언어별로 분리하고 자동 감지와 수동 선택 지원 |

### 그림자 구현 원칙

- 룸·가구처럼 움직이지 않는 환경은 베이크 그림자를 우선한다. 작은 장식물까지 모두 실시간 그림자를
  투영하게 하지 않는다.
- 사용자 존재감은 카메라에 신체 모델을 직접 표시하지 않고, HMD와 양손 추적을 따르는 머리·몸통·팔의
  단순 `Shadow Proxy`를 두고 Renderer의 `Shadows Only`를 사용하는 방안을 우선 검토한다.
- 발·다리 위치를 신뢰할 수 없는 상태에서 완전한 전신 IK 그림자를 만들지 않는다. 잘못된 전신 실루엣은
  존재감을 높이기보다 어색함과 멀미를 유발할 수 있다.
- 현재 정적 조사에서는 PPE 룸의 천장 Spot Light 두 개가 비활성이고 Shadow Type이 `None`이며,
  Quest Mobile URP에서는 Additional Light Shadows가 비활성이다. 사용자 실시간 그림자는 별도 추가광을
  무작정 켜기보다 메인 조명 하나를 기준으로 검증한다.
- 거울 OFF를 기준 실행으로 삼고, 거울 ON에서 반사 카메라와 그림자 비용이 더해졌을 때의 GPU 프레임 시간,
  그림자 흔들림, Quest 양안과 주변 시야를 별도로 비교한다.

### 착용 햅틱 구현 원칙

- 햅틱 발생 기준은 PPE 선택 시도가 아니라 기존 상태 소유자가 착용 성공을 확정한 순간이다.
- 잘못된 PPE, 이미 착용된 PPE, 미리보기·호버 또는 원거리 카드 입력에는 성공 진동을 발생시키지 않는다.
- 실제 선택을 수행한 좌·우 컨트롤러만 짧게 1회 진동하게 하고, 강도와 지속시간은 직렬화된 설정으로
  조정한다. 임의의 상수로 런타임 UI·입력 동작을 확장하지 않는다.
- 구현 전 `입력 장치 → Interactor → Select/Grab → 착용 처리 → 완료 이벤트 → 햅틱 출력` 전체 경로와
  좌·우 컨트롤러 소유권을 먼저 확인한다.

### 체크리스트와 미니 컨트롤러 배치 원칙

- 체크리스트는 현재보다 작게 만들고 사용자 시야 좌측을 따라오게 한다. 미니 컨트롤러 안내는 우측으로
  이동한다.
- 크기, 오프셋, 앵커, 피벗, 추적 속도와 거리 등 표현값은 씬·프리팹·Inspector의 직렬화 값으로 관리한다.
  런타임 코드에서 고정 좌표나 크기를 덮어쓰지 않는다.
- 기존 체크 상태, PPE 착용 갱신, 모달, 텔레포트, 음성, 거울과 XR 입력 흐름은 보존한다.
- 수정 전에 대상 오브젝트의 전체 계층, 부모 Canvas, 월드 위치, RectTransform 코너와 런타임 할당 경로를
  `PPEObjectSpatialDiagnosticHarness` 등 진단 전용 도구로 확인한다.

### 영어 버전과 언어 선택 원칙

- 영문 전용 씬을 복제하지 않는다. 동일 씬과 UI가 고정된 문자열 키를 참조하고, 한국어·영어 문자열 표와
  언어별 Sprite·AudioClip을 선택하도록 구성한다.
- 현재 `Packages/manifest.json`에는 Unity Localization 패키지가 없다. 구현을 시작할 때 Unity 6000.4.8f1과
  호환되는 공식 패키지 버전을 확인하고, 기존 UI 직렬화값을 보존하는 범위에서 도입한다.
- 최초 실행 언어 우선순위는 `저장된 사용자 선택 → Quest 디바이스 언어 → 영어 fallback`으로 한다.
  한국어 locale은 한국어, 지원하지 않는 locale은 영어로 표시하며 앱 안에 `한국어 / English` 선택기를 둔다.
- 국가 IP로 언어를 선택하지 않는다. VPN·회사망·오프라인·실제 선호 언어 불일치와 개인정보 검토 부담이
  있으며, Meta도 Quest 디바이스 언어와 앱 내부 선택기를 기준으로 안내한다.
- Meta Language Pack은 선택 기능이다. 첫 영문판은 문자열과 로고를 기본 APK에 포함해 오프라인과 심사
  환경에서도 즉시 표시하고, TTS 총용량을 확인한 뒤 `ko-kr.lang`, `en-us.lang`으로 음원을 분리하는
  혼합안을 우선 검토한다.
- 언어팩을 사용하면 앱 내부에 목록 조회, 다운로드, 진행·실패·재시도, 설치 확인과 적용 기능이 필요하다.
  언어팩 등록만으로 Quest가 앱 언어를 자동 전환한다고 가정하지 않는다.
- Developer Dashboard의 `앱에서 지원되는 언어`, 영문 앱 이름·설명·검색어·이미지와 실제 앱 지원 범위를
  일치시킨다. 모바일 상세페이지의 팀 이름과 앱 상세의 게시자 이름도 공식 회사명으로 맞춘다.

### 버전 선택안

| 안 | 첫 업데이트 | 다음 업데이트 | 장단점 |
| --- | --- | --- | --- |
| 분리 출시안 | 피드백 1~3을 `1.0.1 / code 10` | 영어판을 `1.1.0 / code 11` | Quest 회귀 범위가 작고 원인 분리가 쉬움 |
| 통합 출시안 | 피드백 1~3과 영어판을 `1.1.0 / code 10` | 후속 수정은 code 11 이상 | 한 번에 공개할 수 있으나 조명·UI·현지화를 함께 검증해야 함 |

- 어느 안을 선택해도 다음 APK/AAB의 `versionCode`는 10이다.
- 현재 논의만으로 통합 출시안을 확정하지 않는다. 사용자가 출시 범위를 결정한 뒤에만
  `bundleVersion`과 `AndroidBundleVersionCode`를 수정한다.

### 역할 분담과 할 일

| 담당 | 할 일 | 상태 |
| --- | --- | --- |
| 사용자 | 공식 영문 프로그램명·회사명·슬로건 확정 | 대기 |
| 사용자 | 화학안전·PPE 영문 용어와 전체 번역 초안 최종 승인 | 대기 |
| 사용자 | 확정 영문 TTS 음원 제공 | 대기 |
| 사용자 | 분리 출시안과 통합 출시안 중 범위 결정 | 대기 |
| 사용자 | Quest에서 그림자 존재감, 햅틱 강도, 체크리스트 위치와 양안 체감 확인 | 구현 후 대기 |
| 사용자 | Meta Dashboard에서 회사명 반영, 지원 언어, 현지화 메타데이터와 최종 출시 승인 | 일부 완료·후속 확인 필요 |
| Codex | 활성 씬·프리팹·런타임 코드의 한글 문자열, 이미지와 음성 참조 전수조사 | 예정 |
| Codex | 고정 문자열 키, 한·영 대조표, 개발자 문맥과 TTS 파일명 목록 작성 | 예정 |
| Codex | 룸·가구 및 사용자 Shadow Proxy의 최소 성능안을 구현하고 비교 검증 | 승인된 구현 단계에서 예정 |
| Codex | 착용 성공 이벤트와 좌·우 입력 소유권을 확인한 뒤 햅틱 연결 | 승인된 구현 단계에서 예정 |
| Codex | 체크리스트·미니 컨트롤러를 씬 작성값 중심으로 재배치 | 승인된 구현 단계에서 예정 |
| Codex | 디바이스 언어 감지, 영어 fallback, 수동 선택·저장과 언어별 UI/TTS 연결 | 승인된 구현 단계에서 예정 |
| Codex | 필요 시 Meta 언어팩 패키지, 앱 내 다운로드·적용 흐름과 업로드 명령 준비 | TTS 용량 결정 후 예정 |

### 서버·텔레메트리 영향

- 현재 계획은 서버 API나 데이터 계약을 변경하지 않는다. 언어 선택값을 새 텔레메트리로 수집하는 기능도
  사용자 요청 없이 추가하지 않는다.
- 표시 버전이 `1.1.0`으로 변경되면 기존 `appVersion` 필드에 새 문자열이 기록될 수 있으므로 서버 저장과
  대시보드의 버전 필터·집계가 새 값을 그대로 표시하는지 확인한다. 이는 계약 변경이 아니라 새 버전 값에
  대한 통합 확인 항목이다.
- 클라이언트 구현, 서버 반영과 실제 Meta Production/운영 데이터 확인을 각각 분리해 기록한다.

### 검증 게이트

1. **정적 확인:** 문자열·이미지·음성 누락, 씬·프리팹 참조, 런타임 UI 덮어쓰기, C# 컴파일과 패키지 상태를
   확인한다.
2. **Unity Editor 확인:** 한국어·영어 즉시 전환, 재시작 선택 유지, UI 잘림·겹침, 착용 성공 1회 햅틱 호출,
   체크리스트 추적과 그림자 ON/OFF 비교를 확인한다.
3. **Quest/OpenXR 확인:** 한국어·영어·미지원 기기 언어, 양안, 실제 컨트롤러 햅틱, 사용자 그림자 정렬,
   거울 OFF/ON 성능과 오프라인 동작을 확인한다.
4. **Meta 확인:** Alpha 또는 별도 테스트 채널에서 현지화 QA를 수행하고, Store 지원 언어·영문 메타데이터·
   팀 회사명·빌드 번호가 실제 앱과 일치하는지 확인한 뒤 Production을 결정한다.
5. 상위 단계의 증거가 없으면 하위 단계 PASS를 Quest 또는 Meta 출시 완료로 확대해 보고하지 않는다.

### 다음 착수 순서

1. 사용자가 분리 출시와 통합 출시 중 범위를 결정한다.
2. Codex가 프로젝트 전체의 표시 문자열·로고·이미지·TTS 참조 목록과 한·영 번역 초안을 만든다.
3. 사용자가 영문 브랜드명과 안전 용어를 승인하고 TTS 제작 대본을 확정한다.
4. 그림자, 햅틱, 추적 UI와 현지화를 한 소비자씩 구현하고 각 단계의 정적·Editor 검증을 분리한다.
5. TTS 총용량을 측정해 기본 APK 포함 또는 Meta Language Pack 분리를 결정한다.
6. 최종 선택에 따라 `1.0.1 / code 10` 또는 `1.1.0 / code 10`으로 빌드하고 Quest 현지화 QA를 수행한다.
7. Meta Dashboard의 회사명·지원 언어·현지화 메타데이터를 확인한 뒤 제출·출시 여부를 사용자가 결정한다.

### Meta 공식 참고 자료

- 현지화 원칙과 기기 언어: `https://developers.meta.com/horizon/design/localization/`
- 언어 기본값 권고: `https://developers.meta.com/horizon/resources/vrc-quest-functional-13/`
- Quest 언어팩: `https://developers.meta.com/horizon/documentation/unity/ps-language-packs/`
- 현지화 QA: `https://developers.meta.com/horizon/resources/testing-localization/`
- 앱 메타데이터: `https://developers.meta.com/horizon/resources/publish-app-metadata/`

### 2026-09-22 후속: PPE 착용 성공 컨트롤러 햅틱 1차 구현

#### 이번 변경이 대응하는 요청과 보존 범위

- 사용자가 PPE 착용 처리 순간 컨트롤러에 진동이 오도록 요청했다.
- 변경 대상은 정상 PPE의 `UseApproved` 확정 시점에 발생하는 햅틱 1회뿐이다. 기존 Grab 판정,
  몸 근접 Activate/Trigger, 오디오, 체크리스트, 장비 표시, 모달, 텔레포트와 거울 동작은 변경하지 않는다.
- 현재 PPE 룸의 `PPEActionPanelController` 25개는 모두 몸 근접 착용 흐름을 사용하고, XRI의 일반
  Select/Hover 햅틱은 꺼져 있다. 따라서 잡는 순간이 아니라 착용 성공 결과가 확정된 단일 지점에 연결했다.

#### 적용 내용

- `PPEActionPanelController.ResolveChoice`에서 `PPEActionChoice.Use`와
  `PPEActionResult.UseApproved`가 함께 확정된 경우에만 `PlayWearHaptic`을 호출한다.
- `PPEInspectionState.TryGetSelectingHandedness`로 현재 PPE를 잡고 있는 손을 확인하고,
  XRI 3.4.1의 `HapticsUtility`를 사용해 왼손은 왼쪽 컨트롤러, 오른손은 오른쪽 컨트롤러에만 진동을 보낸다.
- 오염 PPE 사용 거절, 폐기, 확인하기, Hover와 단순 Grab에는 착용 성공 진동을 보내지 않는다.
- 설정은 각 패널의 Inspector에서 조정할 수 있는 직렬화 값으로 두었다. 3차 체감 보정 후 현재 값은 활성,
  강도 `0.85`, 지속시간 `0.08초`다. 런타임 코드가 씬 작성값을 덮어쓰지 않는다.
- `PPELocomotionPpeRegressionValidationHarness`에 모든 PPE 패널의 햅틱 활성 여부, 강도·지속시간 범위,
  `UseApproved` 전용 게이트와 좌·우 컨트롤러 매핑을 검사하는 항목을 추가했다.

#### 검증 결과와 남은 확인

- **정적 확인:** `dotnet build Assembly-CSharp.csproj --no-restore`와
  `dotnet build Assembly-CSharp-Editor.csproj --no-restore`가 기존 참조 충돌 경고만 남기고 오류 0개로
  완료됐다. `git diff --check`도 공백 오류 없이 통과했다.
- **Unity Editor 확인:** 열린 Unity 6000.4.8f1 Editor 로그에서 스크립트 재컴파일과 Domain Reload 완료를
  확인했고 `error CS`는 없었다. Editor 메뉴의 PPE 회귀 하네스 실행과 실제 착용 흐름 Play Mode 재현은
  아직 수행하지 않았다.
- **Quest/OpenXR 확인:** 실제 좌·우 컨트롤러의 진동 발생, 강도·지속시간 체감과 정상 착용당 정확히 1회인지
  여부는 물리 Quest에서 확인해야 한다. 이 확인 전에는 햅틱 체감까지 정상이라고 보고하지 않는다.

### 2026-09-22 후속: 체크리스트·미니 컨트롤러 재배치 및 출시 버전 확정

#### 확정 결정

- 사용자가 이번 피드백 반영판을 통합 업데이트로 결정했다. 앞의 분리 출시안과 미확정 표기는 이 결정으로 대체하며,
  다음 출시 버전은 `1.1.0 / versionCode 10`으로 확정한다.
- 클라이언트 `ProjectSettings/ProjectSettings.asset`에 `bundleVersion: 1.1.0`,
  `AndroidBundleVersionCode: 10`을 적용했다. 기존 code 9 Alpha 업로드는 그대로 두고 다음 바이너리부터 code 10을 사용한다.

#### UI 재배치 적용

- 대상 씬은 현재 빌드에 포함된 `Assets/Scenes/4_PPE_Room.unity` 하나로 제한했다.
- 기존 `ControllerGuide_mini`의 카메라 자식 World Space Canvas를 공용 머리 고정 HUD 루트로 유지하고,
  컨트롤러 안내 내용인 `Context`만 우측에 배치해 조이스틱 클릭으로 표시·숨김 되도록 참조를 바꿨다.
- `CheckList`는 고정형 `Window Canvas`에서 위 HUD Canvas 아래로 이동했다. 따라서 별도 런타임 위치 보정 없이
  HMD 시선을 따라가며, 좌측 배치와 `0.65` 배율을 씬 작성값으로 저장했다.
- 체크리스트 표시 조건, PPE 착용 체크 갱신, 컨트롤러 입력, 음성 흐름, 모달, 텔레포트, 거울과 장비 처리에는
  동작 변경을 추가하지 않았다. `PPEVoiceFlowDirector`와 `ControllerGuideMiniActivator`는 기존 활성화 로직을
  `ControllerGuide_mini/Context`에 적용한다.

#### 근본 원인과 영향 범위

- 기존 미니 컨트롤러는 `Main Camera`의 자식이라 이미 시선을 따라갔지만 체크리스트는 월드 고정형
  `Window Canvas`의 자식이었다. 단순 좌표 교환만으로는 체크리스트 추적 요구를 만족할 수 없었다.
- 새 Canvas나 런타임 하드코딩을 추가하지 않고 기존 카메라 고정 Canvas를 재사용해, 변경 범위를 씬 계층·작성값과
  두 기존 직렬화 참조로 제한했다.

#### 검증 결과와 남은 수동 검증

- **정적 확인:** 씬 diff에서 기존 FileID만 재사용했고 새 임의 FileID를 만들지 않았다. 부모·자식 참조,
  활성 상태, 좌우 앵커, 체크리스트 배율, 음성 디렉터와 토글 대상 참조를 대조했다. `git diff --check`를 통과했다.
- **C# 확인:** `dotnet build Assembly-CSharp.csproj --no-restore`와
  `dotnet build Assembly-CSharp-Editor.csproj --no-restore`를 각각 순차 실행해 오류 0개를 확인했다.
  기존 어셈블리 참조 충돌 경고는 남아 있다.
- **회귀 하네스:** `PPELocomotionPpeRegressionValidationHarness`에 머리 고정 HUD Canvas, 우측 미니 컨트롤러,
  좌측 축소 체크리스트와 직렬화 참조 검사를 추가했다.
- **Unity Editor 확인:** 사용자가 씬을 다시 불러온 뒤 열린 씬이 `Assets/Scenes/4_PPE_Room.unity`,
  `1.1.0 / versionCode 10`인지 확인했다. 머리 고정 HUD의 `Main Camera` 부모, 체크리스트 좌측 위치·`0.65` 배율,
  미니 컨트롤러 우측 위치와 초기 비활성 상태를 대조했고 `PPELocomotionPpeRegressionValidationHarness`가 PASS했다.
- **Quest/OpenXR 확인 필요:** 양안에서 체크리스트가 좌측에 읽기 좋은 크기로 따라오는지, 미니 컨트롤러가 우측에
  겹침 없이 표시되는지, 조이스틱 클릭 토글과 PPE 착용 체크가 유지되는지 물리 Quest에서 확인해야 한다.

#### 서버 영향

- API, 인증, 데이터 계약과 텔레메트리 필드는 변경하지 않았다. 서버 코드는 `main@0839002234fe` 기준 그대로이며,
  이번 변경은 클라이언트 적용과 공용 회의록 미러링만 해당한다.

### 2026-09-22 후속: PPE 룸 가구·사용자 그림자 1차 구현

#### 변경 요구와 보존 범위

- 사용자가 PPE 룸의 가구뿐 아니라 화면에 보이지 않는 사용자에게도 그림자가 필요하다고 확정했다.
- 변경 대상은 `Assets/Scenes/4_PPE_Room.unity`의 그림자 표현으로 한정했다. PPE Grab·착용 판정,
  햅틱, 체크리스트, 컨트롤러 안내, 음성, 모달, 텔레포트, 거울, 서버 연동 계약은 변경하지 않았다.
- 기존 Inspector·씬 작성값을 보존하기 위해 가구 원본 Transform과 Unlit 재질은 수정하지 않고,
  별도의 씬 작성형 `Shadows Only` 프록시와 바닥 수신 재질을 사용했다.

#### 근본 원인

- 룸에는 활성 그림자 광원이 없었다.
- 활성 Renderer는 대부분 `Cast Shadows`·`Receive Shadows`가 켜져 있었지만, 바닥·벽·주요 가구가
  `Universal Render Pipeline/Unlit` 재질을 사용해 실제 `ShadowCaster` 패스와 그림자 수신 계산이 없었다.
- 따라서 Light만 추가하는 방식으로는 실제 렌더에서 거의 차이가 없었고, 기존 밝은 Unlit 외형을 유지하면서
  그림자만 수신하는 프로젝트 전용 셰이더가 필요했다.

#### 적용 내용

- Quest 모바일 URP의 기존 메인 라이트 그림자 경로만 사용하는 실시간 Directional Light
  `PPE Main Shadow Light` 1개를 추가했다. Additional Light Shadows와 Soft Shadows는 켜지 않았다.
- 바닥에는 `Chemical Safety VR/PPE Unlit Shadow Receiver`를 적용했다. 기존 바닥 텍스처·밝기는 유지하고
  메인 라이트의 그림자 감쇠만 반영한다. 벽과 천장은 과장된 투영을 막기 위해 기존 Unlit 재질을 유지한다.
- 주요 가구 14개에는 원본 Renderer나 Collider를 바꾸지 않는 씬 작성형 `Shadows Only` 프록시를 추가했다.
  프록시는 원본 Bounds에 맞춘 단순 박스로, 화면에는 표시되지 않고 바닥 그림자만 만든다.
- `XRPlayerShadowProxy`는 Main Camera와 좌·우 Controller Transform, 작성된 Floor Renderer를 기준으로
  머리·몸통·양팔·양다리 6개 프록시를 `LateUpdate`에서 배치한다. 프록시는 화면에 보이지 않고
  사용자 그림자만 만든다.
- 사용자 프록시는 Edit Mode에서 비활성으로 저장하고, Play Mode에서 추적 높이가 유효할 때만 켠다.
  필수 참조가 없으면 런타임 자동 생성·자동 수리 없이 오류를 한 번 기록하고 정지한다.
- 전용 셰이더의 Forward 패스에는 Single Pass Instanced용 instance/stereo 매크로를 모두 포함했다.
- `Tools > PPE > Configure Room Shadows`와 `Tools > PPE > Validate Room Shadows`를 추가했다.
  구성 메뉴는 씬 오브젝트와 직렬화 참조를 작성하고, 검증 메뉴는 단일 광원, 모바일 URP 설정,
  바닥 수신 재질, 14개 가구 프록시, 사용자 프록시, Collider 부재와 XR 셰이더 매크로를 검사한다.

#### 검증 결과와 남은 수동 검증

- **정적 확인:** `git diff --check`를 통과했다.
  `dotnet build Assembly-CSharp.csproj --no-restore`와
  `dotnet build Assembly-CSharp-Editor.csproj --no-restore -p:BuildProjectReferences=false`는
  기존 참조·Source Generator 경고만 남기고 오류 0개로 완료됐다.
- **Unity Editor 확인:** 초기 구성 상태에서 전용 `PPERoomShadowValidationHarness`가 PASS했고, 그림자 OFF/ON
  렌더를 비교해 가구 바닥 그림자와 머리·몸통·양팔·양다리 사용자 실루엣이 생성되는 것을 확인했다.
  벽까지 그림자를 받게 한 초기안은 과장된 큰 그림자를 만들어 제외하고 바닥 수신으로 제한했다.
  이후 사용자 프록시를 Edit Mode 비활성·원점 Transform으로 저장하고 정렬 검사를 강화한 최신 상태에서는
  C# 컴파일과 정적 직렬화 값만 확인했다. 전용 하네스 재실행은 Unity 연결이 약 90초 동안 응답하지 않아
  중단했으므로 최신 Editor PASS로 확대하지 않는다.
- **통합 하네스 미완료:** `PPELocomotionPpeRegressionValidationHarness`는 Preview Scene 재개방 단계에서
  Unity MCP 호출이 300초를 초과해 완료 결과를 받지 못했다. 이 결과를 PASS로 기록하지 않는다.
  전용 그림자 하네스와 C# 컴파일 결과만 확인된 상태다.
- **Play Mode·Quest/OpenXR 확인 필요:** Editor를 다시 연 뒤 실제 HMD 위치에서 사용자 그림자가 발밑에
  정렬되는지, 양손 이동이 실루엣에 반영되는지, 양안 표시와 거울 OFF/ON GPU 프레임 시간,
  그림자 강도·흔들림을 Quest에서 확인해야 한다.

#### 서버 영향

- API, 인증, 데이터 계약, 텔레메트리와 대시보드에는 변경이 없다.
- 이 공용 문서의 그림자 구현 상태만 서버 저장소의 동일 상대 경로에 미러링한다.

### 2026-09-22 후속: 코드·VR 최적화 범위와 출시 안정성 결정

#### 결정 배경

- 사용자는 기존 출시 일정에 영향을 줄 수 있는 코드 최적화와 VR 최적화를 출시 전에 일괄 적용하지 않았다.
- 광범위한 리팩터링이나 렌더링 구조 변경은 성능을 개선할 가능성이 있지만, PPE 상태 전이, XR 입력,
  씬 로딩, 양안 렌더링과 기존 직렬화 참조에 회귀를 만들 수 있다. 측정 자료 없이 변경 범위를 넓히는 것은
  출시 안정성보다 위험이 크다고 판단했다.
- 다음 업데이트 `1.1.0 / versionCode 10`은 승인된 실무자 피드백인 착용 햅틱, 체크리스트·미니 컨트롤러
  재배치와 PPE 룸 그림자에 집중한다. 프로젝트 전체 코드 리팩터링이나 전면적인 VR 렌더링 최적화는
  이번 업데이트의 필수 범위에 포함하지 않는다.

#### 최적화의 구분

- **코드 최적화:** 중복 실행, 불필요한 매 프레임 처리, 반복 할당과 GC, 과도한 물리 검사처럼 실제 병목으로
  확인된 코드를 줄이는 작업이다. 단순한 코드 정리나 구조 변경만으로 성능 향상을 가정하지 않는다.
- **VR 최적화:** Quest에서 양쪽 눈을 안정적으로 렌더링하기 위해 조명·그림자, 셰이더, 드로우콜,
  오버드로우, 모델 복잡도, 거울용 추가 카메라, 메모리와 물리 비용을 조정하는 작업이다.
- 현재 그림자 구현은 메인 그림자 광원 1개, Additional/Soft Shadow 비활성, 단순 `Shadows Only` 프록시,
  바닥 한정 수신 방식으로 비용을 제한했다. 다만 이것은 설계상 제한이며, 실제 Quest 성능 검증을 대신하지 않는다.

#### 이후 진행 원칙

1. 동일한 Quest와 동일한 씬·동작 구간에서 변경 전 기준 프레임 시간, CPU/GPU 병목, 메모리와 순간 끊김을
   먼저 측정한다. PPE 룸 기본 상태, 그림자 ON/OFF와 거울 OFF/ON을 분리해 비교한다.
2. 측정으로 확인된 병목 하나만 한 번에 수정한다. 요청하지 않은 입력 소비자, PPE 상태 전이, UI 표시 조건,
   음성·오디오와 서버 연동 동작은 최적화를 이유로 변경하지 않는다.
3. 각 최적화는 기능 피드백 변경과 별도 커밋·검증 단위로 관리하고, 성능 향상 수치와 화질·동작 차이를 함께 기록한다.
4. 정적·C# 검증, Unity Editor, Play Mode, Quest/OpenXR 양안 검증을 구분한다. Game View나 컴파일 성공만으로
   Quest 최적화가 완료됐다고 판단하지 않는다.
5. 실제 성능 문제가 없거나 개선 폭이 검증되지 않으면 `1.1.0 / versionCode 10` 출시 일정에 대규모 최적화를
   추가하지 않는다. 성능 병목이 확인되면 후속 업데이트 범위와 회귀 검증 시간을 별도로 산정한다.

#### 현재 상태와 남은 확인

- **완료:** 최적화의 의미, 이번 업데이트에서 보존할 기능 범위와 측정 기반 후속 원칙을 확정했다.
- **미수행:** 프로젝트 전체 코드 프로파일링, Quest GPU/CPU 프레임 분석과 메모리 장시간 측정은 아직 수행하지 않았다.
- **필수 수동 확인:** 그림자 적용 PPE 룸에서 머리 회전·이동 시 끊김, 양안 표시, 거울 OFF/ON 성능을 실제
  Quest에서 비교한다. 문제가 발견되면 출시 직전의 포괄적 리팩터링 대신 해당 병목만 수정한다.

#### 서버 영향

- 이번 결정과 그림자 최적화 원칙은 클라이언트 실행 성능에 관한 것으로, 서버 API, 인증, 데이터 계약,
  텔레메트리와 대시보드를 변경하지 않는다.
- 공용 회의록만 서버 저장소의 동일 상대 경로에 미러링하며, 서버 코드 반영이나 통합 검증 완료로 확대해 기록하지 않는다.

### 2026-09-22 후속: PPE 룸 그림자 형태·각도·농도 2차 보정

#### 피드백과 근본 원인

- Game View 확인에서 행거와 벤치 아래 그림자가 가구 형태보다 큰 직사각형 덩어리처럼 보였고 농도도 진했다.
- 1차 구현은 Quest 비용을 줄이기 위해 가구 하나당 Renderer 전체 Bounds를 감싼 Cube 프록시 1개를 사용했다.
  이 방식은 드로우콜은 적지만 행거 틈, 벤치 다리와 장비 외곽이 모두 채워져 실루엣이 부자연스러웠다.
- 주광원 X 회전 `72°`도 거의 수직이라 그림자가 물체 아래에 뭉치고 형태가 읽히기 어려웠다.
- 바닥만 전용 수신 셰이더를 사용하고 벽은 URP Unlit을 유지했기 때문에, 벽에 붙은 마스크 진열장과 열린
  보관함 문의 그림자가 벽 방향으로 투영돼도 화면에는 표시되지 않았다.

#### 적용 내용

- 가구 14개 각각의 활성 MeshFilter 삼각형을 Editor에서 하나의 정적 그림자 메시로 결합했다. 원본 가구,
  머티리얼과 Collider는 변경하지 않고 기존 `Shadows Only` 프록시에 결합 메시만 연결했다.
- 세부 가구가 여러 자식 Renderer로 구성돼도 가구당 그림자 Renderer 1개를 유지한다. 따라서 원본 196개
  Renderer를 그림자 패스에 개별 추가하지 않고 14개 그림자 드로우 단위로 실루엣을 보존한다.
- 결합 그림자 메시 14개의 프로젝트 원본 용량은 약 4.1MB다. 실제 Quest GPU 비용은 별도 기기 측정이 필요하다.
- 처음에는 주광원 회전을 `Quaternion.Euler(58, -35, 0)`으로 낮춰 실루엣을 드러내려 했지만, 쓰레기통
  그림자가 바닥 접점이 아니라 앞쪽에 분리되어 보이는 결과가 확인돼 이 설정을 폐기했다.
- 현재 주광원은 `Quaternion.Euler(90, 0, 0)`의 수직 방향으로 고정했다. `shadowBias`는 `0.015`,
  `shadowNormalBias`는 `0.05`로 낮춰 바닥 그림자가 물체 바로 아래 접점에서 시작하도록 했다.
- Soft Shadow는 Quest 비용 때문에 활성화하지 않았다. 대신 Light의 `shadowStrength`를 `0.72`에서 `0.48`로
  낮춰 경계 계산을 늘리지 않고 그림자 농도를 완화했다.
- 연해진 그림자 안에서 바닥 텍스처의 밝은 얼룩이 구멍처럼 보이지 않도록, 바닥 수신 셰이더는 텍스처에
  어두운 계수를 곱하는 방식 대신 작성된 단일 그림자 색상으로 보간한다. 실제 투명 구멍이나 Soft Shadow는 추가하지 않았다.
- 같은 수신 셰이더를 사용하는 벽 전용 머티리얼 `PPE_Room_Wall_ShadowReceiver`를 추가했다. 기존 벽 텍스처와
  색을 보존하고 벽 그림자 강도는 `0.42`로 별도 제한한다. 다만 바닥 접지를 우선해 주광원을 수직으로 변경했으므로,
  벽에 붙은 진열장과 열린 문 접촉감은 대각선 주광원이 아닌 별도의 정적 접촉 그림자 방식으로 보완해야 한다.
  천장은 기존 Unlit 머티리얼을 유지하므로 천장 덕트가 천장 자체를 과도하게 어둡게 만들지 않는다.
- 천장 덕트는 바닥에서 멀고 현재 룸 셸의 단순화된 천장과 조명 방향 때문에 바닥·벽에 길고 부자연스러운
  그림자를 만들므로 `PPE_B_Duct` Renderer의 `Cast Shadows`를 `Off`로 고정했다. 덕트 위치·재질·수신 설정은 유지한다.
- 결합 그림자 Mesh의 내부 이름을 파일명과 일치시켜 재적용 때 Unity가 출력하던 자산 이름 불일치 경고를 제거했다.
- `Tools > PPE > Refine Room Shadow Appearance` 명시적 Editor 메뉴를 추가했다. 실행 시에만 결합 메시와
  씬 작성값을 갱신하며 런타임에서 프록시를 재생성하거나 자동 수리하지 않는다.

#### 검증 결과와 남은 확인

- **정적·C# 확인:** `Assembly-CSharp-Editor.csproj` 빌드는 기존 Source Generator 경고만 남기고 오류 0개다.
- **Unity Editor 확인:** `PPERoomShadowValidationHarness`가 PASS했다. 가구별 프로젝트 메시, 원본 Bounds 정렬,
  프록시 identity Transform, 바닥·벽 수신/천장 제외, 수직 메인 그림자 광원, `0.48` 강도,
  `0.015/0.05` Bias와 XR 셰이더 매크로를 확인했다.
- **시각 진단:** 별도 비-XR 진단 카메라에서 행거의 옷·프레임, 벤치 상판·다리 사이가 분리된 바닥 실루엣으로
  표시되고 기존 큰 직사각형 그림자가 제거된 것을 확인했다. Main Camera 정적 캡처에서는 벽면 그림자 수신도
  확인했다. 이 결과는 실제 문 동작, Quest 양안·성능 검증을 대신하지 않는다.
- **Quest/OpenXR 확인 필요:** 실제 사용자 시점에서 그림자 길이와 방향, 농도, 사용자 그림자 정렬, 양안 표시와
  거울 OFF/ON GPU 프레임 시간을 비교한다. Soft Shadow 활성화는 이 측정 전에는 후속안으로 채택하지 않는다.

#### 그림자 누락 감사

- 주광원은 위에서 아래로 비추며, 그림자는 광원 반대편 바닥으로 투영되는 현재 Directional Light 회전과 일치한다.
- 주요 가구 14개와 사용자 머리·몸통·양팔·양다리는 전용 프록시가 있다. 천장 덕트는 시각적 부조화와
  불필요한 그림자 비용을 피하기 위해 `Cast Shadows Off`로 제외했다.
- 별도 `PPE` 루트의 장비 23종은 184개 활성 Renderer 중 대부분이 Unlit 또는 Marker 재질이라 전체 그림자가
  없거나 일부 Lit 부품만 그림자를 만든다. 잡아서 이동하는 장비이므로 고정 월드 프록시를 추가하면 그림자가
  원래 자리에 남는다. 후속 구현 시 각 장비 자식의 로컬 결합 프록시가 장비와 함께 이동하도록 해야 한다.
- 고정 소품에서는 `PPE_B_HandSanitizer`, `PPE_B_CCTV_01`, `PPE_B_CCTV_02`가 Unlit이라 자체 그림자가 없다.
  소독기는 보완 우선순위가 높고, 천장 CCTV는 바닥에 불필요하게 긴 그림자가 생길 수 있어 시각 확인 후 결정한다.
- 문 이미지, 배수구, 룸 셸과 평면 거울은 평면·수신면 또는 반사 시스템이므로 그림자 발산 대상에서 제외한 현재 구성이 맞다.

#### 서버 영향

- 클라이언트 씬·Editor 생성 자산 보정이며 서버 API, 데이터 계약, 텔레메트리와 대시보드 변경은 없다.
- 이 공용 기록만 서버 저장소의 같은 상대 경로에 미러링한다.

### 2026-09-22 후속: 컨트롤러 모델 원거리 레이 교정 및 착용 PPE 그림자 연동

#### 변경 요구와 보존할 동작

- 사용자가 손 모델이 아니라 좌·우 컨트롤러 모델에서 나오는 레이의 위치·방향을 맞추고, 방호복과 장화를
  착용했을 때도 사용자 그림자에 해당 실루엣이 나타나도록 요청했다.
- 씬·Inspector 작성값은 기존 컨트롤러, 모델, Interactor와 PPE 원본 Transform을 직접 이동하지 않는 방식으로
  보존했다. 컨트롤러별 레이 시작점 자식과 PPE별 `Shadows Only` 자식만 추가했다.
- 컨트롤러 입력의 단일 기준은 기존 `NearFarInteractor`이며, 모델 시각 정렬 기준은 각 모델의
  `left_laser_begin`·`right_laser_begin`이다. 착용 그림자 상태의 단일 기준은 기존 원본 Renderer의
  `enabled`와 장비 자식의 `activeInHierarchy`다.
- 입력 전체 경로 중 `Controller tracking → NearFarInteractor → CurveInteractionCaster → Far Ray`의 시작점만
  변경했다. `SphereInteractionCaster`의 Near Grab 시작점, `InteractionAttachController`의 부착 기준,
  입력 Action, Layer, Collider, Raycaster와 PPE 착용 판정은 변경하지 않았다.
- 필수 모델 표식, Caster, Mesh 또는 Renderer가 없으면 Editor 구성 메뉴가 오류로 중단한다. 런타임에서
  오브젝트나 참조를 자동 생성·수리하지 않는다.
- 영향 소비자는 컨트롤러 원거리 레이와 착용 사용자 그림자다. UI, 텔레포트, PPE Grab, 거울 카메라,
  음성·오디오와 서버 연동 동작은 보존했다.

#### 적용 내용

- 좌·우 컨트롤러 바로 아래에 `Left Controller Ray Origin`, `Right Controller Ray Origin`을 씬 작성값으로
  추가했다. 컨트롤러 모델은 안내 흐름에서 비활성화될 수 있으므로 새 시작점은 모델 자식으로 두지 않고,
  모델 표식의 월드 Pose를 컨트롤러 로컬 Pose로 변환해 저장했다.
- 각 `CurveInteractionCaster.m_CastOrigin`만 새 시작점에 연결했다. 교정 전 진단 표본은 왼쪽이 모델 표식과
  약 `0.0444 m / 9.87°`, 오른쪽이 약 `0.0411 m / 22.51°` 어긋나 있었다. 첫 적용에서 오른쪽 모델 표식의
  회전까지 복사하자 레이가 위로 휘는 회귀가 발생했고, 진단 결과 오른쪽 표식의 `forward`가 기존 XRI 조준보다
  약 `22.76°` 위를 향하는 것이 원인이었다. 최종 상태는 두 손 모두 모델 표식과 시작 위치 오차 `0.000000 m`,
  왼쪽은 모델 표식 회전 오차 `0.0000°`, 오른쪽은 기존 XRI 조준 회전 오차 `0.0000°`다.
- 착용 사용자 실루엣은 전체 장비 Renderer를 그림자 패스에 중복 투입하지 않고 방호복 본체, 일반 장화 좌·우,
  테이핑 장화 좌·우의 5개 Mesh만 원본 자식 `Shadows Only` 프록시로 추가했다. 원본 Mesh와 로컬 Pose를
  그대로 공유하고 Collider, Light Probe, Reflection Probe와 Motion Vector 비용은 추가하지 않았다.
- `PPEWornEquipmentShadowSync`는 기존 `PPEHazmatEquipController`가 변경하는 Renderer 활성 상태와
  `PPEEquipmentVisualController`가 변경하는 자식 활성 상태를 `LateUpdate`에서 그림자 Renderer에만 반영한다.
  위치·회전·스케일, 착용 상태와 입력을 변경하지 않는다.
- `Tools > PPE > Align Controller Model Rays`, `Validate Controller Model Rays`,
  `Configure Worn PPE Shadows`, `Validate Worn PPE Shadows` 명시적 Editor 메뉴를 추가했다.
- 사용자가 왼쪽 컨트롤러 모델의 위·아래 방향을 조정한 뒤 모델이 레이에서 벗어난 추가 회귀를 확인했다.
  진단 결과 레이 자체가 아니라 왼쪽 모델의 `left_laser_begin`이 기존 레이 시작점에서 `0.1074m` 이동해 있었다.
  승인된 모델 회전과 레이 회전은 모두 보존하고 컨트롤러 모델 루트만 평행 이동해 두 시작점을 다시 맞췄다.
  적용 후 좌·우 위치 오차는 모두 `0.000000m`이며 오른쪽 모델은 변경하지 않았다.
- `Tools > PPE > Align Controller Models To Ray Origins`를 추가했다. 이 메뉴는 모델의 회전·스케일과 레이의
  위치·회전을 변경하지 않고 모델의 레이 표식이 기존 레이 시작점에 닿도록 모델 위치만 이동한다.
  기존 `Align Controller Model Rays`도 이미 작성된 레이 시작점의 회전을 다시 덮어쓰지 않도록 수정했다.

#### 검증 결과와 남은 수동 검증

- **정적·C# 확인:** `Assembly-CSharp-Editor.csproj` 빌드는 기존 참조·Source Generator 경고만 남기고
  오류 0개로 완료됐다. Unity Console에는 오류가 없다.
- **Unity Editor 확인:** 레이 검증은 좌·우 모델 표식과 시작 위치, 작성된 레이 회전 보존,
  오른쪽 기존 XRI 조준 회전, `CurveInteractionCaster` 참조를 확인하고 Near Grab과 부착 기준이
  각 Interactor에 그대로 남았음을 확인해
  PASS했다. 착용 그림자 검증은 5개 원본/프록시
  쌍, Mesh·identity Transform·`Shadows Only` 설정, Collider 부재와 직렬화 참조를 확인해 PASS했다.
  기존 `PPERoomShadowValidationHarness`도 함께 PASS했다.
- **Play Mode 확인:** 방호복 원본 Renderer를 표시하면 그림자 Renderer가 켜지고 숨기면 꺼지는 것을 확인했다.
  장화 자식은 비활성일 때 그림자가 없고 활성일 때 그림자가 켜졌으며, Play Mode에서도 좌·우 레이 시작 위치와
  각 손의 승인된 조준 방향이 유지됐다. 검증 중 변경은 디스크에 저장하지 않고 씬을 다시 열어 `dirty=False`를 확인했다.
- **Quest/OpenXR 확인 필요:** 실제 컨트롤러를 들었을 때 레이가 모델 끝에서 자연스럽게 출발하는지, 방호복·장화
  착용 순서에 따라 발밑 실루엣이 과도하게 겹치지 않는지, 양안 표시와 GPU 프레임 시간을 실기에서 확인해야 한다.
  Game View·Play Mode 확인을 Quest 양안 정상으로 확대하지 않는다.

#### 서버 영향

- 클라이언트 씬·렌더링 표현만 변경했으며 서버 API, 인증, 데이터 계약, 텔레메트리와 대시보드 변경은 없다.
- 이 공용 기록만 서버 저장소의 같은 상대 경로에 미러링한다.

### 2026-09-22 후속: 근거리 그림자 선명도 및 착용 햅틱 강도 2·3차 보정

#### 피드백과 보존 범위

- 사용자는 가구 그림자 크기는 개선됐지만 가까이에서 그림자가 여전히 많이 흐리고, 착용 햅틱은 조금 더
  강해도 된다고 확인했다.
- 그림자의 수직 조명 각도, 결합 Mesh 형태, `0.48` 농도와 `0.015/0.05` Bias는 보존한다. 전역 농도를
  높여 먼 그림자가 다시 진해지는 대신 근거리 Shadow Map 텍셀 배분만 조정한다.
- 햅틱은 기존 `UseApproved` 1회, 선택한 손의 컨트롤러, `0.08초` 지속시간을 보존하고 강도만 변경한다.
  Grab, Hover, 오염 PPE 거절과 확인하기에는 새 진동을 추가하지 않는다.

#### 적용 내용

- `Mobile_RPAsset`의 메인 라이트 Shadow Map 해상도는 Quest 비용을 고려해 `1024`로 유지했다.
- 그림자 거리를 `50m → 40m`, Cascade 수를 `1 → 2`, 근거리 Cascade 분할을 `0.35`로 변경했다.
  룸 활성 Renderer Bounds의 시작 카메라 기준 최대 모서리 거리는 약 `36.09m`이므로 전체 룸 범위를 유지하면서
  가까운 영역에 더 많은 Shadow Map 텍셀을 배분한다.
- Soft Shadow는 계속 비활성이다. 따라서 이번 변경은 그림자 크기·형태·농도를 바꾸지 않고 근거리 경계
  해상도를 높이는 조정이다. 두 Cascade의 추가 ShadowCaster 처리 비용은 실제 Quest에서 측정해야 한다.
- 2 Cascade 적용 뒤에도 가까운 그림자는 형태가 읽히지 않고 먼 거리·거울에서만 진하다는 추가 피드백을 받았다.
  전역 Light 강도 `0.48`과 먼 거리 수신 강도는 유지하고, 수신 셰이더에 카메라 거리 기반 근거리 대비를 추가했다.
  `5m` 안쪽에서 바닥은 최대 `0.40`, 벽은 최대 `0.25`만큼 수신 강도를 보강하며 거리 제곱 기준으로 부드럽게
  감소한다. 따라서 가까운 바닥 실루엣을 우선 또렷하게 하고 먼 거리·거울의 기존 농도 증가는 제한한다.
- PPE 룸 `PPEActionPanelController` 25개의 작성값은 체감 피드백에 따라 `0.45 → 0.65 → 0.85`로 변경했다.
  지속시간은 `0.08초` 그대로이며, `PPEActionPanelController`의 새 컴포넌트 기본값도 `0.85`로 맞췄다.
- `Tools > PPE > Tune Wear Haptics`와 `Validate Wear Haptic Tuning`을 추가했다. 구성 메뉴가 씬 작성값을
  명시적으로 갱신하고 런타임에서는 해당 값을 덮어쓰지 않는다.

#### 검증 결과와 남은 수동 검증

- **Unity Editor 확인:** `PPERoomShadowValidationHarness`, `PPEWearHapticTuningSetup`,
  `PPEControllerModelRayAlignmentSetup`과 `PPELocomotionPpeRegressionValidationHarness`가 PASS했다.
  적용값은 `40m / 2 cascade / split 0.35 / 1024`, 근거리 보강은 바닥 `0.40`·벽 `0.25`·거리 `5m`,
  햅틱은 `0.85 / 0.08초`로 확인했다. Unity Console의 shader·script 오류는 없다.
- **보존 확인:** 오른쪽 레이 시작 위치와 복구된 XRI 조준 방향, Near Grab·부착 기준, 그림자 Light 회전·농도와
  PPE 착용 성공 게이트는 변경하지 않았다.
- **Quest/OpenXR 확인 필요:** 가까운 바닥 그림자 경계가 충분히 선명해졌는지, Cascade 경계가 눈에 띄지 않는지,
  거리 기반 대비가 양안에서 동일한지, GPU 프레임 시간과 햅틱 `0.85`의 실제 체감을 물리 Quest에서 확인해야 한다.
  너무 강하면 강도만 `0.70~0.80` 범위로 낮추고 지속시간과 발생 조건은 유지한다.

#### 서버 영향

- 클라이언트 렌더 파이프라인 작성값과 착용 햅틱 설정만 변경했다. 서버 API, 인증, 데이터 계약,
  텔레메트리와 대시보드에는 영향이 없다.
- 이 공용 기록만 서버 저장소의 같은 상대 경로에 미러링한다.

### 2026-09-22 후속: 체크리스트 최종 배치와 중도 퇴장 미니 가이드 종료

#### 변경 요구와 보존 범위

- Play Mode에서 사용자가 직접 조정한 체크리스트 패널 크기와 내부 항목 위치를 씬 작성값으로 보존하고,
  머리 고정 HUD 안에서 너무 멀거나 정면을 가리지 않도록 크기·거리·좌우 위치를 반복 보정했다.
- 미니 컨트롤러 가이드는 기존 우측 배치와 조이스틱 클릭 토글을 유지한다. 중도 퇴장 지점에 도착했을 때만
  사용자가 직접 끄지 않았더라도 자동으로 숨기도록 종료 상태를 정리했다.
- UI가 물체에 가려졌을 때 뒤로 물러나라는 TTS는 추가하지 않았다. PPE를 잡기 위해 앞으로 이동하는 정상 행동과
  충돌하고, 접근할 때마다 음성이 반복될 수 있다는 사용자 판단에 따라 런타임 연결 대상에서 제외했다.
- 체크리스트 표시 조건, PPE 착용 판정과 갱신, 컨트롤러 입력, 텔레포트, 모달, 장비 처리와 음성 순서는 변경하지 않았다.

#### 최종 씬 작성값

- 머리 고정 HUD `ControllerGuide_mini`는 `Main Camera`의 직접 자식이며 World Space Canvas,
  로컬 Z `0.30m`, 로컬 스케일 `0.0008`을 사용한다.
- 미니 컨트롤러 콘텐츠 `Context`는 우측 작성 위치 `anchoredPosition=(525, 0)`을 유지한다.
- 체크리스트 `CheckList`의 최종 작성값은 `anchoredPosition=(-867.825, -32)`, 로컬 Z `76`,
  로컬 스케일 `1.04`다. 카메라 기준 실측 위치는 약 `(-0.69, -0.01, 0.36)m`로, 정면을 가렸던
  중간값 약 `(-0.46, -0.01, 0.36)m`보다 다시 좌측으로 이동했다.
- 체크리스트 패널은 사용자가 Play Mode에서 조정한 `sizeDelta=(466.4, 647.9)`와 내부 항목 위치를 보존했다.
  밀폐공간 `Confined`와 누출 `Leak` 그룹은 내용에 맞게 서로 다른 작성 배치를 유지하며, 런타임이나 하네스가
  두 그룹의 좌표를 강제로 같게 만들지 않는다.
- UI 배치는 런타임 코드에 하드코딩하지 않았다. 전체 관련 RectTransform 경로를 Play Mode에서 캡처한 뒤
  Edit Mode 씬 값으로 적용했고, `Awake`, `OnEnable`, `Start`, 활성화 도우미와 토글 코드가 Transform을
  덮어쓰지 않는 것을 확인했다.

#### 중도 퇴장 처리

- `PPEVoiceFlowDirector.NotifyMidExitArrived()` 진입 직후
  `SetActive(m_ControllerGuideMini, false)`를 호출한다.
- 미니 가이드 숨김은 중도 퇴장 전용 Voice 참조 검사보다 먼저 실행된다. 따라서 해당 음원이 누락된 상태에서도
  미니 가이드는 남지 않는다.
- `PPELocomotionPpeRegressionValidationHarness`는 이 호출 순서와 최종 HUD·체크리스트 작성값을 함께 검사한다.

#### 검증 결과

- **정적 확인:** 체크리스트와 미니 가이드의 런타임 Transform 대입 경로가 없고, 중도 퇴장 숨김이 Voice
  누락 검사보다 먼저 실행되는 것을 소스와 회귀 하네스로 확인했다.
- **Unity Editor 확인:** `PPELocomotionPpeRegressionValidationHarness.Validate()`가 PASS했다. 최종 씬 값 저장 후
  Play Mode에 다시 진입해 체크리스트의 위치 `(-867.825, -32)`, Z `76`, 스케일 `1.04`와 카메라 기준 거리
  약 `0.36m`가 덮어써지지 않는 것을 확인했다. Unity Console 오류는 없다.
- **Quest/OpenXR 확인 필요:** 실제 양안에서 체크리스트가 정면 작업 영역을 가리지 않으면서 읽기 쉬운지,
  머리 회전 중 흔들림·눈 피로가 없는지, 중도 퇴장 시 미니 가이드가 즉시 사라지는지 확인해야 한다.

#### 후속 작업

1. Quest 실기에서 체크리스트 최종 위치·크기·가독성, 우측 미니 컨트롤러와의 좌우 균형을 확인한다.
2. 중도 퇴장 지점에서 미니 가이드를 켠 상태로 진입해 자동 숨김과 전용 음성 흐름을 확인한다.
3. 실제 컨트롤러로 PPE 착용 성공 햅틱 `0.85 / 0.08초`를 확인하고 과하면 강도만 `0.70~0.80`으로 낮춘다.
4. 방호복·장화 착용 전후 사용자 그림자, 가구 그림자, 거울 반사 농도와 컨트롤러 레이 좌우 정렬을 양안에서 확인한다.
5. Quest Profiler에서 그림자 2 Cascade 적용 전후 GPU 프레임 시간과 발열을 비교한다. 성능 문제가 확인될 때만
   그림자 거리·Cascade·프록시 범위를 한 항목씩 조정한다.
6. 영어판은 동일 씬 기반 Localization 구조로 진행한다. 영문 로고·UI 문자열·이미지 텍스트와 사용자가 준비할
   영문 TTS를 연결하고, `저장된 선택 → Quest 기기 언어 → 영어 fallback`과 앱 내 수동 언어 선택을 검증한다.
7. 위 Quest 회귀가 끝난 뒤 다음 업데이트를 `1.1.0 / versionCode 10`으로 빌드하고 Meta 채널 업로드 전
   Android Release·서명·양안·입력·오디오·성능 게이트를 다시 실행한다.
8. 현재 `origin/main` 푸시가 끝난 뒤 새 계정의 비공개 GitHub 저장소로 전체 Git 이력을 복제하고 그 저장소를
   새 작업 기준으로 전환한다. 대상 URL과 접근 권한을 확인한 뒤 브랜치·태그·Git LFS 사용 여부를 함께 점검한다.
   GitHub Actions Secret, 배포 키, 브랜치 보호 규칙과 협업 권한은 Git 복제로 자동 이전되지 않으므로 새 저장소에서
   별도로 재설정하고, 기존 원격은 검증이 끝날 때까지 삭제하지 않는다.

#### 서버 영향과 공용 문서 동기화

- 이번 변경은 클라이언트 UI 배치와 중도 퇴장 표시 상태만 다룬다. 서버 API, 인증, 데이터 계약,
  텔레메트리와 대시보드 변경은 없다.
- 이 공용 회의록은 서버 저장소의 같은 상대 경로에 미러링한다. 서버 코드 반영이나 통합 검증 완료로 확대해 기록하지 않는다.
