...
## 2026-09-06 추가 정정: 관통과 Game View 검정화면 분리 유지

### 최신 사용자 확인

- 사용자는 몸통 보행 Collider는 듣지만 손 모델과 HMD 시점이 뚫린다고 확인했다.
- 별도로, 파란 주변 시야 깨짐과 검정화면은 관통 때만 발생하는 것이 아니며 Unity Game View도 Play 지속 중
  검정화면이 된다고 확인했다.

### 판단 경계

- HMD·손 관통은 XR Origin 또는 CharacterController 이동이 아니라 실제 추적 Pose가 환경 Collider를 우회하는
  문제로 계속 추적한다.
- Game View 검정화면이 동반되는 시야 깨짐은 관통 대응의 직접 결과나 Quest Link/HMD 전송 문제로 단정하지
  않고, 렌더링·Camera·OpenXR 세션 안정성 문제로 별도 문서에서 추적한다.
- 관통 대응을 구현할 때 Game View 검정화면 해결까지 완료됐다고 보고하지 않는다.
- 렌더링 안정화 조사를 진행할 때도 PPE Grab 보존, HMD 침범 감지, 손 표시 제한 설계를 섞어 변경하지 않는다.
