...
## 2026-09-10 Meta 제출 이미지 v2 16:9 출력

### 적용한 변경

- 사용자 지정 원본 `C:/Users/lanoc/Downloads/metahorizon_title_v2 (.png`를 중앙 기준 최소 크롭
  `(x=2, y=0, width=1045, height=588)` 후 2560×1440으로 확대해
  `C:/Users/lanoc/Downloads/metahorizon_title_v2_2560x1440.png`로 출력했다.
- 사용자 지정 원본 `C:/Users/lanoc/Downloads/safety_training_vr_1 (1).png`를 중앙 기준 최소 크롭
  `(x=6, y=0, width=1036, height=583)` 후 2560×1440으로 확대해
  `C:/Users/lanoc/Downloads/safety_training_vr_1_2560x1440_v2.png`로 출력했다.
- 두 출력 모두 원본 구도·문자·VR 로고·PPE와 배경을 재해석하지 않는 고품질 보간 결과이며 24-bit RGB
  PNG다. 기존 동명 2560×1440 파일은 덮어쓰지 않았다.

### 검증과 제한

- 두 파일의 실제 픽셀 크기 2560×1440과 `Format24bppRgb`를 확인하고 렌더링된 전체 화면에서 문자·로고·
  PPE 잘림과 새 요소 추가가 없음을 시각 검토했다.
- `metahorizon_title_v2_2560x1440.png`의 SHA-256은
  `E775F0B81EA67E906C4443638ECE12E89FE6B13F1112985AE32325E555E73743`이다.
- `safety_training_vr_1_2560x1440_v2.png`의 SHA-256은
  `10ADB7DBB667148613336BEE1EBAB195FE7963402598157EF38D74B9982098A6`이다.
- 이미지 편집 후보는 장비 세부·프레이밍을 재해석해 제외했다. 최종 파일은 생성형 후보가 아니라 원본 픽셀
  기반의 결정론적 출력이다.
- 결과물은 사용자 Downloads에 있으며 Git 제출물에는 포함하지 않는다. Meta Dashboard 업로드 후의 실제
  압축·크롭·표시 결과는 아직 수동 확인이 필요하다.
